...
export const getPerfilParticipante = async (id:number) => {
    ...
    FROM dev.participante p
    --JOIN dev.estatus_participante e ON p.estatus_participante_id = e.id
    JOIN dev.pais c ON p.pais_id = c.id
    LEFT JOIN dev.estado s ON p.estado_id = s.id
    JOIN dev.genero g ON p.genero_id = g.id
    JOIN dev.talla_playera t ON p.talla_playera_id = t.id
    JOIN dev.carrera cr on p.carrera_id = cr.id
    LEFT JOIN dev.semestre sm on p.semestre_id = sm.id
    LEFT JOIN dev.universidad u ON p.universidad_mexico_id = u.id
    WHERE p.id = $1; `;
